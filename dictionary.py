...
class Dictionary:
    ...
    def diff_suppr(self, other):
        l = sorted(list(set(other.l)-set(self.l)))
        return l
    
    def diff_add(self, other, nb_letters=0, letters=''):
        l = []
        if nb_letters > 1:
            l = list(set(self.wordlist(nb_letters, letters)) - set(other.wordlist(nb_letters, letters)))
        else: # 0 means all words
            for key in self.d.keys():
                l.extend(list(set(self.wordlist(key, letters)) - set(other.wordlist(key, letters))))
        l = sorted(l)  
        return l
